        ...
        <div class="main-wrapper">
                ...
                <div class="header-right">
                    <div class="user-mini">
                        <div class="user-info" style="text-align:right;">
                            <p class="name"><?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Admin'); ?></p>
                            <p class="role"><?php
                                $role = $_SESSION['actual_role'] ?? $_SESSION['role'] ?? 'admin';
                                echo $role === 'super admin' ? 'Super Administrator' : 'Administrator';
                            ?></p>
                        </div>
                        <div class="avatar"><?php
                            $nameParts = explode(' ', trim($_SESSION['full_name'] ?? 'AD'));
            ...
            <main class="content-body">
                ...
                <div id="dashboard" class="view-section">
                    ...
                    <!-- Welcome Bar -->
                    <div style="display:flex; align-items:center; justify-content:space-between; background:#fff; border:1px solid #e2e8f0; border-radius:10px; padding:14px 20px; margin-bottom:0;">
                        <div>
                            <h4 style="margin:0; font-size:1.05rem; font-weight:700; color:#0f172a;">Selamat Datang, <?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Admin'); ?> 👋</h4>
                            <p style="margin:0; font-size:0.82rem; color:#64748b; margin-top:2px;"><?php
                                $hari = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'];
                                $bulan = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
                                echo $hari[date('w')] . ', ' . date('j') . ' ' . $bulan[date('n')-1] . ' ' . date('Y') . ' — Ringkasan aktivitas sistem hari ini.';
                            ?></p>
                        </div>
                    ...
                    <div class="grid-2">
                            ...
                            <div class="card-body" style="max-height: 280px; overflow-y: auto; padding-right: 10px;">
                                <div style="font-size: 0.9rem; color: #334155; display: flex; flex-direction: column; gap: 15px;">
                                    ...
                                    <div style="display: flex; gap: 10px; align-items: flex-start;">
                                        ...
                                        <div>
                                            <strong style="color: #0f172a;">Analisis Ringkasan Dashboard</strong>
                                            <ul style="margin: 4px 0 0 15px; padding: 0; list-style-type: disc;">
                                                <li><strong>Kartu Statistik</strong> di atas diperbarui otomatis setiap halaman dibuka. Klik <strong>Refresh</strong> untuk memuat ulang data terkini.</li>
                                                <li><strong>Tugas Aktif</strong> — jumlah jadwal yang sedang berjalan hari ini (belum melewati tanggal akhir).</li>
                                                <li><strong>Menunggu Approval</strong> — laporan kerja yang dikirim pegawai dan perlu ditinjau admin.</li>
                                                <li><strong>Hadir Hari Ini</strong> — jumlah karyawan yang sudah Clock-In beserta info yang terlambat.</li>
                                            </ul>
                                        </div>
                                    </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Navigasi Cepat</h3>
                                <p class="card-sub">Akses langsung ke menu utama sistem.</p>
                            </div>
                            <div class="card-body" style="display:flex; flex-direction:column; gap:12px;">

                                <!-- Monitoring Kerja -->
                                <div onclick="navigateTo('monitoring')" style="border:1.5px solid #e2e8f0; border-radius:10px; padding:14px 16px; cursor:pointer; display:flex; align-items:center; gap:14px; transition:all 0.2s; background:#fff;" onmouseover="this.style.borderColor='#3b82f6';this.style.background='#f8faff'" onmouseout="this.style.borderColor='#e2e8f0';this.style.background='#fff'">
                                    <div style="width:42px;height:42px;background:#eff6ff;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                        <i data-lucide="map-pin" style="width:20px;color:#2563eb;"></i>
                                    </div>
                                    <div style="flex:1;">
                                        <div style="font-weight:700;color:#0f172a;font-size:0.9rem;">Monitoring Kerja</div>
                                        <div style="font-size:0.78rem;color:#64748b;margin-top:2px;">Pantau laporan &amp; aktivitas lapangan</div>
                                    </div>
                                    <div style="display:flex;align-items:center;gap:8px;">
                                        <span id="qnav-pending" style="background:#f59e0b;color:white;padding:2px 9px;border-radius:20px;font-size:0.72rem;font-weight:700;display:none;">0</span>
                                        <i data-lucide="chevron-right" style="width:16px;color:#94a3b8;"></i>
                                    </div>
                                </div>

                                <!-- Penjadwalan -->
                                <div onclick="navigateTo('schedule')" style="border:1.5px solid #e2e8f0; border-radius:10px; padding:14px 16px; cursor:pointer; display:flex; align-items:center; gap:14px; transition:all 0.2s; background:#fff;" onmouseover="this.style.borderColor='#16a34a';this.style.background='#f0fdf4'" onmouseout="this.style.borderColor='#e2e8f0';this.style.background='#fff'">
                                    <div style="width:42px;height:42px;background:#f0fdf4;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                        <i data-lucide="calendar" style="width:20px;color:#16a34a;"></i>
                                    </div>
                                    <div style="flex:1;">
                                        <div style="font-weight:700;color:#0f172a;font-size:0.9rem;">Penjadwalan</div>
                                        <div style="font-size:0.78rem;color:#64748b;margin-top:2px;">Buat dan kelola jadwal tugas karyawan</div>
                                    </div>
                                    <div style="display:flex;align-items:center;gap:8px;">
                                        <span id="qnav-tasks" style="background:#16a34a;color:white;padding:2px 9px;border-radius:20px;font-size:0.72rem;font-weight:700;display:none;">0</span>
                                        <i data-lucide="chevron-right" style="width:16px;color:#94a3b8;"></i>
                                    </div>
                                </div>

                                <!-- Riwayat Absensi -->
                                <div onclick="navigateTo('riwayat-absensi-all')" style="border:1.5px solid #e2e8f0; border-radius:10px; padding:14px 16px; cursor:pointer; display:flex; align-items:center; gap:14px; transition:all 0.2s; background:#fff;" onmouseover="this.style.borderColor='#ea580c';this.style.background='#fff7ed'" onmouseout="this.style.borderColor='#e2e8f0';this.style.background='#fff'">
                                    <div style="width:42px;height:42px;background:#fff7ed;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                        <i data-lucide="clock" style="width:20px;color:#ea580c;"></i>
                                    </div>
                                    <div style="flex:1;">
                                        <div style="font-weight:700;color:#0f172a;font-size:0.9rem;">Riwayat Absensi</div>
                                        <div style="font-size:0.78rem;color:#64748b;margin-top:2px;">Pantau kehadiran seluruh karyawan</div>
                                    </div>
                                    <div style="display:flex;align-items:center;gap:8px;">
                                        <span id="qnav-att" style="background:#ea580c;color:white;padding:2px 9px;border-radius:20px;font-size:0.72rem;font-weight:700;display:none;">0</span>
                                        <i data-lucide="chevron-right" style="width:16px;color:#94a3b8;"></i>
                                    </div>
                                </div>
